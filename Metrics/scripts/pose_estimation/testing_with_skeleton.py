...
video_path = r'E:\python\python_projects\yolov7\latest\medias\video\standing_jacks.mp4'  # Replace with the path to your test video
    ...
    while cap.isOpened():
        ...
        if results.pose_landmarks:
            ...
            try:
                ...
                feedback = give_feedback(test_angles, ref_angles)
                if feedback:
                    for message in feedback:
                        cv2.putText(frame, message, (10, 30 + len(feedback)*30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
                else:
                    cv2.putText(frame, "Good form!", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)

                # Draw the pose landmarks (skeleton) on the frame
