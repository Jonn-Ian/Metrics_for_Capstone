...
# Function to generate feedback based on angle differences
def generate_joint_feedback(test_angles, ref_angles):
    ...
    # Provide feedback based on the joint with the largest discrepancy
    if max_diff_index == 0:
...
while cap.isOpened() and loop_count < 2:
    ...
    if recognized_name == "Unknown":
        print("Unknown face detected. Exiting...")
    ...
    if results.pose_landmarks:
        ...
        if frame_index < len(df):  # Ensure frame_index is within the CSV data length
            reference_angles = df.iloc[frame_index][['left_elbow_angle', 'right_elbow_angle', 'left_knee_angle', 'right_knee_angle']].values
            similarity = compute_similarity(current_angles, reference_angles)
            total_similarity += similarity
            num_frames += 1

            # Generate feedback based on angle differences
            feedback = generate_joint_feedback(current_angles, reference_angles)

            # Display feedback on the video
            cv2.putText(frame, feedback, (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

    # Display the resulting frame
    cv2.imshow('Pose Estimation and Face Recognition', frame)
