...
# Video feed (use the camera instead of a video file)
...
while cap.isOpened() and loop_count < 2:
    ...
    if results.pose_landmarks:
        ...
        # Show the pose landmarks on the frame
        if results.pose_landmarks:
            mp.solutions.drawing_utils.draw_landmarks(
                frame, results.pose_landmarks, mp_pose.POSE_CONNECTIONS)

    # Display the frame with annotations (pose and face recognition)
    cv2.imshow('Exercise Monitoring with Face and Pose Detection', frame)

    # Exit the loop if 'q' is pressed
    if cv2.waitKey(1) & 0xFF == ord('q'):
        break

    # Update frame index for feedback timing
    frame_index += 1
